    ...
    async def get_dynamic_model(cls, metadata_service: MetadataService) -> type["TopNDocumentsModel"] | None:
        ...
        if sortable_fields := metadata_service.get_sortable_fields():
            ...
            return create_model(
                cls.__name__,
                __base__=cls[
                    Literal[tuple(map(itemgetter(0), sortable_fields))]
                ],
                __doc__=cls.__doc__,
            )
        return None
...
class DocumentMatcherConfig(BaseModel, ABC):
    ...
    async def get_dynamic_model(cls) -> type["DocumentMatcherConfig"]:
        ...
        return create_model(
            cls.__name__,
            filters=(list[single_filter_model], Field(
                default=[],
                description=(
            ...
            top_n=(top_n_model | None if top_n_model is not None else None, Field(
                default=None,
...
class DocumentMatcher:
    ...
    def _get_field_filtering_clause(self, name: str, value: Any, field_info: FieldInfo) -> ColumnElement[bool] | None:
    ...
    def _get_field_max_value_clause(self, name: str, filter_clauses: list[ColumnElement[bool]]) -> ColumnElement[bool]:
        """
        ...
        return DocumentEntity.metadata_[key].astext.in_(
            select(
                func.max(DocumentEntity.metadata_[key].astext)
            ).where(
                DocumentEntity.channel_key == self._channel_key,
                *filter_clauses
            ).having(
                func.max(DocumentEntity.metadata_[key].astext).is_not(None)
            )
        )
        ...
        return (
            select(
                ...
                *order_by
                ...
                bindparam("top_n", self._config.top_n.limit)
            )
        )
